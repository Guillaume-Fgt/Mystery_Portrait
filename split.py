...
def split(im, rows, cols, split_dir):
    im_width, im_height = im.size
    row_width = int(im_width / rows)
    row_height = int(im_height / cols)
    n = 0
    current_directory = os.getcwd()
    split_dir = f"{current_directory}/{split_dir}"
...
def main():
    ...
    parser.add_argument("image_path", nargs=1, help="The path of the image to process.")
    parser.add_argument(
        "rows",
        type=int,
        default=2,
    ...
    if args.reverse:
        ...
        reverse_split(paths_to_merge, args.rows, args.cols, image_path, args.cleanup)
    else:
